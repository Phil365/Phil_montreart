    ...
    <?php echo "<div class='dummy'><!--    Ne mettez rien ici--></div>";
            ...
            ?>
                    ...
                    <form id='formContribuerPhoto' action="?r=oeuvre&o=<?php echo($idOeuvreencours);?>&action=envoyer" onsubmit="return validePhotoSubmit();" method="post" enctype="multipart/form-data">
                        <h3>Avez vous prise une photographie de cette oeuvre d'art public?</h3><h4>Contribuyez une photographie et contribuyez à Montréart! </h4>
                        <input class='#' type="file" name="fileToUpload" id="btnFileToUpload" value="Sélectionner une image...">
                        <input class='boutonMoyenne' id="btnContribuerPhoto" type="submit" value="Téléverser Image" name="submit">
                    </form>
            <?php
                ...
                <p class='noIndent'>".$this->oeuvre["description" . $this->langue]."</p></div>";
                    ...
                    if ($this->commentaires) {//Si des commentaires existent pour cette oeuvre dans la langue d'affichage...
                        for ($i = 0; $i < count($this->commentaires); $i++) {
                            ...
                            echo "<img class='thumbnail' src = 'images/$imgPhoto'>";
                            echo "<h4 id='idUtilisateur'>".$this->commentaires[$i]["nomUsager"]."</h4>";
