<?php
   ...
   */   
    ...
    if(empty($_GET['rAjax'])) {//Requête ajax d'une page
		$_GET['rAjax'] = '';
	}
    if(empty($_GET['o'])) {//id d'une oeuvre
		$_GET['o'] = '';
	}
    if(empty($_GET['action'])) {//action à effectuer
		$_GET['action'] = '';
	}
    ...
    if(empty($_GET['testAutocomplete'])) {//initialise les keywords de l'autocomplete (test)
